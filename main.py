...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = 'Привет! Купи ' + animal[sessionStorage[user_id]['animal']] + 'а!'
        # Получим подсказки
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response']['text'] = animal[sessionStorage[user_id]['animal']] + 'а можно найти на Яндекс.Маркете!'
        if not animal[sessionStorage[user_id]['animal']]:
            res['response']['end_session'] = True
        animal[sessionStorage[user_id]['animal']] += 1
        return
    # Если нет, то убеждаем его купить слона!
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {animal[sessionStorage[user_id]['animal']]}а!"
    res['response']['buttons'] = get_suggests(user_id)
